...
\end{align}$$
 ...
 $$1-\beta \approx P \left( \bigcap_{k=1}^{3} Z_{k}^{\dagger}>z_{k}^{\dagger} \mid \boldsymbol{\delta}^{*} \right) = \Phi_{2}\left( -z_{1}^{\dagger}, -z_{2}^{\dagger},-z_{3}^{\dagger} ; \Gamma_{123} \right) $$
...
$1-\beta$ is the required power, n is the number of patients per group, $z_{\alpha}$ is the $(1- \alpha)100^{th}$ standard normal percentile and $\Gamma_{123}$ is the correlation matrix.
